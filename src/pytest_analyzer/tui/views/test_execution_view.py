...
class TestExecutionView(Widget):
    ...
    async def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "run_tests_button":
            ...
            log_widget.clear()
            log_widget.write("Preparing to start test execution...")
            progress_bar = self.query_one(ProgressBar)
            ...
            if not current_test_target:
                log_widget.write(
                    "[bold red]Error: Test target not set in the app.[/bold red]"
                )
            ...
            log_widget.write(f"Target for execution: {current_test_target}")

            if hasattr(self.app, "test_execution_controller") and hasattr(
                self.app.test_execution_controller, "execute_tests_async"
            ):
                # The TestExecutionController.execute_tests_async handles UI updates
                ...
                await self.app.test_execution_controller.execute_tests_async(
                    str(current_test_target)
                )
            else:
                log_widget.write(
                ...
                self.app.notify(
                    "Test execution controller not configured.", severity="error"
                )
                progress_bar.visible = (
                    False  # Hide progress bar as execution cannot start
                )

    def update_progress(
        self, current: float, total: float, description: str = ""
    ) -> None:
        progress_bar = self.query_one(ProgressBar)
