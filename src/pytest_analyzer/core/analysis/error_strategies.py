"""
Error analysis strategies for FailureAnalyzer and FixSuggester.

Implements the Strategy pattern to encapsulate error-type-specific logic.
"""

import re
from typing import List, Protocol

from ..models.pytest_failure import FixSuggestion, PytestFailure


class ErrorAnalysisStrategy(Protocol):
    """
    Protocol for error analysis strategies.
    """

    def analyze(self, failure: PytestFailure) -> List[FixSuggestion]: ...


class AssertionErrorStrategy:
    """Strategy for analyzing AssertionError failures."""

    def analyze(self, failure: PytestFailure) -> List[FixSuggestion]:
        suggestions = []
        if "assert" in failure.traceback:
            suggestion, confidence = self._analyze_assert_statement(failure)
            if suggestion:
                suggestions.append(
                    FixSuggestion(
                        failure=failure, suggestion=suggestion, confidence=confidence
                    )
                )
        if not suggestions:
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion="Review the expected vs. actual values in the assertion.",
                    confidence=0.5,
                )
            )
        return suggestions

    def _analyze_assert_statement(self, failure: PytestFailure):
        equality_match = re.search(
            r"assert\s+([^=]+)\s*==\s*([^,]+)", failure.traceback
        )
        if equality_match:
            left = equality_match.group(1).strip()
            right = equality_match.group(2).strip()
            expected_match = re.search(
                r"E\s+assert\s+([^=]+)\s*==\s*([^,]+)", failure.traceback
            )
            if expected_match:
                actual = expected_match.group(1).strip()
                expected = expected_match.group(2).strip()
                suggestion = f"Change the test to expect {actual} instead of {expected}, or fix the code to return {expected}."
                return suggestion, 0.8
            return f"Check why {left} != {right}.", 0.6
        in_match = re.search(r"assert\s+([^\s]+)\s+in\s+([^,]+)", failure.traceback)
        if in_match:
            item = in_match.group(1).strip()
            container = in_match.group(2).strip()
            return f"Ensure that {item} is present in {container}.", 0.7
        true_match = re.search(r"assert\s+([^,]+)\s*(?:is True)?", failure.traceback)
        if true_match:
            condition = true_match.group(1).strip()
            return f"Ensure that {condition} evaluates to True.", 0.6
        return "", 0.0


class AttributeErrorStrategy:
    """Strategy for analyzing AttributeError failures."""

    def analyze(self, failure: PytestFailure) -> List[FixSuggestion]:
        suggestions = []
        attr_match = re.search(
            r"'(.+)'\s+object\s+has\s+no\s+attribute\s+'(.+)'", failure.error_message
        )
        if attr_match:
            obj_type = attr_match.group(1)
            attribute = attr_match.group(2)
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion=f"Check if '{attribute}' is misspelled or if there's a similar attribute in '{obj_type}'.",
                    confidence=0.7,
                )
            )
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion=f"Add the '{attribute}' attribute to the '{obj_type}' class.",
                    confidence=0.6,
                )
            )
        else:
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion="Check if the attribute exists and is spelled correctly.",
                    confidence=0.5,
                )
            )
        return suggestions


class ImportErrorStrategy:
    """Strategy for analyzing ImportError failures."""

    def analyze(self, failure: PytestFailure) -> List[FixSuggestion]:
        suggestions = []
        module_match = re.search(r"No module named '(.+)'", failure.error_message)
        if module_match:
            module = module_match.group(1)
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion=f"Install the '{module}' module with pip or conda.",
                    confidence=0.7,
                )
            )
            if "." in module:
                suggestions.append(
                    FixSuggestion(
                        failure=failure,
                        suggestion=f"Check the relative import path for '{module}'.",
                        confidence=0.6,
                    )
                )
        else:
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion="Check if the module exists and is in the Python path.",
                    confidence=0.5,
                )
            )
        return suggestions


class TypeErrorStrategy:
    """Strategy for analyzing TypeError failures."""

    def analyze(self, failure: PytestFailure) -> List[FixSuggestion]:
        suggestions = []
        arg_match = re.search(
            r"(\w+)\(\) (takes|got)\s+(\d+)\s+(\w+)\s+but\s+(\d+)\s+(\w+)",
            failure.error_message,
        )
        if arg_match:
            func_name = arg_match.group(1)
            expected = arg_match.group(3)
            actual = arg_match.group(5)
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion=f"Check the arguments passed to '{func_name}()'. It expected {expected} arguments but got {actual}.",
                    confidence=0.8,
                )
            )
        kwarg_match = re.search(
            r"got an unexpected keyword argument '(.+)'", failure.error_message
        )
        if kwarg_match:
            param = kwarg_match.group(1)
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion=f"Remove or rename the '{param}' keyword argument.",
                    confidence=0.8,
                )
            )
        mult_match = re.search(
            r"can't multiply sequence by non-int of type '(.+)'", failure.error_message
        )
        if mult_match:
            type_name = mult_match.group(1)
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion=f"Convert the {type_name} to an integer before using it as a multiplier.",
                    confidence=0.7,
                )
            )
        if not suggestions:
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion="Check the types of the arguments and return values.",
                    confidence=0.5,
                )
            )
        return suggestions


class NameErrorStrategy:
    """Strategy for analyzing NameError failures."""

    def analyze(self, failure: PytestFailure) -> List[FixSuggestion]:
        suggestions = []
        name_match = re.search(r"name '(.+)' is not defined", failure.error_message)
        if name_match:
            var_name = name_match.group(1)
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion=f"Check if '{var_name}' is misspelled or if it needs to be defined.",
                    confidence=0.7,
                )
            )
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion=f"Ensure '{var_name}' is imported or defined before use.",
                    confidence=0.6,
                )
            )
        else:
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion="Check if the variable is defined in the current scope.",
                    confidence=0.5,
                )
            )
        return suggestions


class IndexErrorStrategy:
    """Strategy for analyzing IndexError failures."""

    def analyze(self, failure: PytestFailure) -> List[FixSuggestion]:
        suggestions = []
        if "list index out of range" in failure.error_message:
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion="Check the length of the list and ensure the index is within range.",
                    confidence=0.7,
                )
            )
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion="Add a bounds check before accessing the list element.",
                    confidence=0.6,
                )
            )
        else:
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion="Check the index and ensure it's within the range of the sequence.",
                    confidence=0.5,
                )
            )
        return suggestions


class KeyErrorStrategy:
    """Strategy for analyzing KeyError failures."""

    def analyze(self, failure: PytestFailure) -> List[FixSuggestion]:
        suggestions = []
        key_match = re.search(r"KeyError: '?([^']+)'?", failure.error_message)
        if key_match:
            key = key_match.group(1)
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion=f"Check if '{key}' exists in the dictionary before accessing it.",
                    confidence=0.7,
                )
            )
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion=f"Use dict.get('{key}', default_value) to provide a default value.",
                    confidence=0.6,
                )
            )
        else:
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion="Check if the key exists in the dictionary before accessing it.",
                    confidence=0.5,
                )
            )
        return suggestions


class ValueErrorStrategy:
    """Strategy for analyzing ValueError failures."""

    def analyze(self, failure: PytestFailure) -> List[FixSuggestion]:
        suggestions = []
        int_match = re.search(
            r"invalid literal for int\(\) with base (\d+): '(.+)'",
            failure.error_message,
        )
        if int_match:
            base = int_match.group(1)
            value = int_match.group(2)
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion=f"Ensure '{value}' is a valid integer in base {base}.",
                    confidence=0.7,
                )
            )
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion="Add validation or error handling for non-integer inputs.",
                    confidence=0.6,
                )
            )
        if not suggestions:
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion="Check the value being processed and ensure it meets the expected format.",
                    confidence=0.5,
                )
            )
        return suggestions


class SyntaxErrorStrategy:
    """Strategy for analyzing SyntaxError failures."""

    def analyze(self, failure: PytestFailure) -> List[FixSuggestion]:
        suggestions = []
        if "invalid syntax" in failure.error_message:
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion=f"Check the syntax at line {failure.line_number}.",
                    confidence=0.7,
                )
            )
        if failure.relevant_code:
            if "(" in failure.relevant_code and ")" not in failure.relevant_code:
                suggestions.append(
                    FixSuggestion(
                        failure=failure,
                        suggestion="Check for missing closing parenthesis ')'.",
                        confidence=0.8,
                    )
                )
            if "[" in failure.relevant_code and "]" not in failure.relevant_code:
                suggestions.append(
                    FixSuggestion(
                        failure=failure,
                        suggestion="Check for missing closing bracket ']'.",
                        confidence=0.8,
                    )
                )
            if "{" in failure.relevant_code and "}" not in failure.relevant_code:
                suggestions.append(
                    FixSuggestion(
                        failure=failure,
                        suggestion="Check for missing closing brace '}'.",
                        confidence=0.8,
                    )
                )
            if ":" not in failure.relevant_code and (
                "if " in failure.relevant_code
                or "for " in failure.relevant_code
                or "while " in failure.relevant_code
                or "def " in failure.relevant_code
            ):
                suggestions.append(
                    FixSuggestion(
                        failure=failure,
                        suggestion="Add a colon ':' at the end of the statement.",
                        confidence=0.8,
                    )
                )
        if not suggestions:
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion="Check the syntax of the code.",
                    confidence=0.5,
                )
            )
        return suggestions


class GenericErrorStrategy:
    """Strategy for analyzing generic/unrecognized errors."""

    def analyze(self, failure: PytestFailure) -> List[FixSuggestion]:
        suggestions = []
        suggestions.append(
            FixSuggestion(
                failure=failure,
                suggestion=f"Investigate the {failure.error_type} error at line {failure.line_number if failure.line_number else 'unknown'}.",
                confidence=0.5,
            )
        )
        if failure.relevant_code:
            suggestions.append(
                FixSuggestion(
                    failure=failure,
                    suggestion=f"Review the code: {failure.relevant_code}",
                    confidence=0.4,
                )
            )
        return suggestions
