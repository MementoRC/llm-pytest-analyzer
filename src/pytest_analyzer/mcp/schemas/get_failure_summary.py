...
class GetFailureSummaryRequest:
    ...
    def validate(self) -> List[str]:
        ...
        if self.group_by not in valid_group_by:
            errors.append(
                f"Invalid group_by '{self.group_by}'. Must be one of: {valid_group_by}"
            )
        ...
        if self.time_range not in valid_time_ranges:
            errors.append(
                f"Invalid time_range '{self.time_range}'. Must be one of: {valid_time_ranges}"
            )
...
class GetFailureSummaryResponse:
    ...
    def _compute_summary_stats(self):
        ...
        self.summary_stats = {
            ...
            "average_failures_per_file": self.total_failures / len(file_counts)
            if file_counts
            else 0,
        }
