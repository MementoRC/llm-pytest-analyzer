...
class AnalyzePytestOutputRequest:
    ...
    def validate(self) -> List[str]:
        ...
        if self.filter_by_type is not None:
            valid_types = {"assertion_error", "exception", "import_error", "syntax_error", "timeout"}
            for failure_type in self.filter_by_type:
                if failure_type not in valid_types:
                    errors.append(f"Invalid failure type '{failure_type}'. Must be one of: {valid_types}")
...
class AnalyzePytestOutputResponse:
    ...
    def _calculate_average_confidence(self) -> float:
        ...
        total_confidence = sum(suggestion.confidence_score for suggestion in self.suggestions)
        return total_confidence / len(self.suggestions)
    
    def get_high_confidence_suggestions(self, threshold: float = 0.8) -> List[FixSuggestionData]:
