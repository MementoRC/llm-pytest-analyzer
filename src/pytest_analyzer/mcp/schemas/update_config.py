...
class UpdateConfigRequest(MCPRequest):
    ...
    def validate(self) -> List[str]:
        ...
        if self.merge_strategy not in valid_strategies:
            errors.append(f"Invalid merge_strategy '{self.merge_strategy}'. Must be one of: {valid_strategies}")
        ...
        if "llm" in self.config_updates:
            ...
            if "temperature" in llm_config:
                ...
                if not isinstance(temp, (int, float)) or not (0.0 <= temp <= 2.0):
                    errors.append("llm.temperature must be a number between 0.0 and 2.0")
