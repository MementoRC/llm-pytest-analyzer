...
def _parse_version(version_str: str) -> Tuple[int, ...]:
    ...
    clean_version = version_str.split('.dev')[0].split('rc')[0].split('a')[0].split('b')[0]
    try:
    ...
    try:
        ...
        if files:
            ...
            for file in files:
                ...
                elif any(part in str(file_path) for part in ['etc', 'usr']) and 'bin' not in str(file_path):
                    suspicious_paths.append(str(file_path))
...
def validate_dependencies() -> None:
    ...
    for dep in required_deps:
        ...
        try:
            ...
            logger.debug(f"Required dependency '{dep}' (import '{import_name}') is missing.")
    ...
    if security_issues:
        error_msg = (
            "Security: The following security issues were found:\n"
...
def get_dependency_report() -> Dict[str, any]:
    ...
    for dep in all_deps:
        ...
        try:
            ...
            if dep in ["pydantic", "rich", "structlog", "prometheus_client", "mcp", "httpx"]:
