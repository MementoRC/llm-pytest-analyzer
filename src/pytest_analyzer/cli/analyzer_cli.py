...
def display_suggestions(suggestions: List[FixSuggestion], args):
    ...
    if not filtered_suggestions and suggestions:
        ...
        source = "LLM" if best_suggestion.code_changes and best_suggestion.code_changes.get('source') == 'llm' else "Rule-based"
    ...
    for fingerprint, group in suggestions_by_fingerprint.items():
        ...
        if args.verbosity >= 2 and fingerprint and fingerprint.startswith(('AttributeError', 'ImportError', 'TypeError')):
...
def apply_suggestions_interactively(
    ...
    for i, suggestion in enumerate(suggestions):
        ...
        file_changes = {k: v for k, v in suggestion.code_changes.items() 
        ...
        console.print(f"[bold]Failure:[/bold] {suggestion.failure.test_name if hasattr(suggestion.failure, 'test_name') else 'Unknown'}")
