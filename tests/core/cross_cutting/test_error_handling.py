...
def test_error_context_does_not_double_wrap(caplog):
    ...
    assert (
        exc_info.value is original_custom_exc
...
def test_error_handler_uses_default_logger_if_none_provided(caplog):
    ...
    assert (
        f"Calling function 'my_func_default_logger' for operation: {op_name}"
        in caplog.text
    )
    # Verify the logger name by checking record.name
    assert any(
        record.name == error_handling_module_logger.name and op_name in record.message
...
def test_batch_operation_all_success(caplog):
    ...
    results, errors = batch_operation(
        items, sample_op_success, op_name, logger=test_logger
    )
    ...
    assert (
        f"Batch operation '{op_name}' completed. Successful: 3, Failed: 0."
        in caplog.text
    )
...
def test_batch_operation_stops_on_first_error_if_continue_on_error_false(caplog):
    ...
    assert "Error processing item 'fail_item'" in caplog.text
    assert (
        f"Batch operation '{op_name}' stopped early due to error on item 'fail_item'"
        in caplog.text
    )
...
def test_batch_operation_all_failures(caplog):
    ...
    results, errors = batch_operation(
        items, op_func_all_fail, op_name, logger=test_logger
    )
    ...
    assert (
        f"Batch operation '{op_name}' completed. Successful: 0, Failed: 2."
        in caplog.text
    )
...
def test_batch_operation_empty_list(caplog):
    ...
    assert (
        f"Batch operation '{op_name}' completed. Successful: 0, Failed: 0."
        in caplog.text
    )
...
def test_batch_operation_preserves_original_error_details_in_tuple(caplog):
    ...
    results, errors = batch_operation(
        items, op_func_specific_error, op_name, logger=test_logger
    )
    ...
    assert "Error processing item 'specific_error_item'" in caplog.text
