...
def test_cli_with_assertion_file(
    ...
    with patch("pathlib.Path.exists", return_value=True):
        with patch("builtins.open", mock_open(read_data=json_content)):
            ...
            # Get captured output but don't need to use it here
            _ = f.getvalue()
...
def test_cli_with_report_file(sample_json_report):
    ...
    # We don't need to use error_output here
    _ = f_err.getvalue()

    # Check if we already have the expected output
    if not (
        "Suggested fix" in output
        and ("test_assertion" in output or "AssertionError" in output)
    ):
