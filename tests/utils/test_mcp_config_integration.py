...
class TestSettingsMCPIntegration:
    ...
    def test_settings_custom_mcp(self):
        ...
        custom_mcp = MCPSettings(
            transport_type="http", http_port=9000, enable_authentication=True
        )
...
class TestConfigurationManagerMCPIntegration:
    ...
    def test_env_variables_override_yaml(self):
        ...
        try:
            with patch.dict(os.environ, env_vars, clear=False):
                ...
                assert (
                    settings.mcp.transport_type == "http"
                )  # From env, not stdio from YAML
                assert settings.mcp.http_port == 9000  # From env, not 8000 from YAML
                assert (
                    settings.mcp.enable_authentication is True
                )  # From env, not False from YAML
    ...
    def test_partial_mcp_configuration(self):
        ...
        config_data = {"mcp": {"transport_type": "http", "http_port": 8080}}
        ...
        env_vars = {"PYTEST_ANALYZER_MCP_ENABLE_AUTHENTICATION": "true"}
    ...
    def test_mcp_settings_yaml_validation_error(self):
        ...
        config_data = {"mcp": {"transport_type": "invalid", "http_port": -1}}
