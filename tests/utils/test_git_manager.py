...
def test_init_git_repository_success(mock_subprocess, mock_project_dir):
    ...
    expected_calls = [
        call(['git', 'init'], cwd=mock_project_dir, capture_output=True, check=True, text=True),
        call(['git', 'add', '.'], cwd=mock_project_dir, capture_output=True, check=True, text=True),
        call(['git', 'commit', '-m', 'Initial commit by pytest-analyzer'], cwd=mock_project_dir, 
...
def test_create_branch_for_fixes_success(mock_subprocess, mock_project_dir):
    ...
    mock_subprocess.assert_has_calls([
        call(['git', 'rev-parse', '--abbrev-ref', 'HEAD'], 
             cwd=str(mock_project_dir), capture_output=True, check=True, text=True),
        call(['git', 'checkout', '-b', 'fix-branch'], 
             cwd=str(mock_project_dir), capture_output=True, check=True, text=True)
    ])
...
def test_create_branch_for_fixes_auto_name(mock_subprocess, mock_project_dir):
    ...
    with patch('time.time', return_value=12345):
        ...
        mock_subprocess.assert_has_calls([
            call(['git', 'rev-parse', '--abbrev-ref', 'HEAD'], 
                cwd=str(mock_project_dir), capture_output=True, check=True, text=True),
            call(['git', 'checkout', '-b', 'pytest-analyzer-fixes-12345'], 
                cwd=str(mock_project_dir), capture_output=True, check=True, text=True)
        ])
...
def test_create_branch_for_fixes_error(mock_subprocess, mock_project_dir):
    ...
    mock_subprocess.side_effect = [
        success_result,
        subprocess.CalledProcessError(1, ['git', 'checkout', '-b'], stderr="fatal: A branch named 'fix-branch' already exists.")
    ]
...
def test_commit_fix_success(mock_subprocess, mock_project_dir):
    ...
    with patch('pathlib.Path.exists', return_value=True):
        ...
        assert result is True
        mock_subprocess.assert_has_calls([
            call(['git', 'add', str(test_file.resolve())], 
                cwd=str(mock_project_dir), capture_output=True, check=True, text=True),
            call(['git', 'commit', '-m', f"fix: Apply pytest-analyzer fix for Test issue in {test_file.name}"], 
                cwd=str(mock_project_dir), capture_output=True, check=True, text=True)
        ])
...
def test_commit_fix_add_error(mock_subprocess, mock_project_dir):
    ...
    mock_subprocess.side_effect = subprocess.CalledProcessError(
        1, ['git', 'add'], stderr="fatal: pathspec 'test_file.txt' did not match any files")
...
def test_commit_fix_commit_error(mock_subprocess, mock_project_dir):
    ...
    mock_subprocess.side_effect = [
        success_result,
        subprocess.CalledProcessError(1, ['git', 'commit'], stderr="fatal: unable to auto-detect email address")
    ]
...
def test_reset_file_error(mock_subprocess, mock_project_dir):
    ...
    mock_subprocess.side_effect = subprocess.CalledProcessError(
        1, ['git', 'checkout'], stderr="error: pathspec 'test_file.txt' did not match any file(s) known to git")
